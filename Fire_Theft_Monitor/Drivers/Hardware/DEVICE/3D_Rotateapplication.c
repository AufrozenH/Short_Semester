...
{																	
  ...
  {2,2,8},
	{6,2,8},	//只这是正方体特征点坐标,相当于正方形的边长，可以增加一个函数来修改长短，以及9变成长方形或其他的
	{2,6,8},	//后面有些参数没有单独剥离出来，所以此处修改后，有些地方可能也要相应修改。请实际操作进行修改。
	{6,6,8}	//在arduino的游戏机中看到过有画三维旋转飞机的，原理上应该一样，将飞机的各个点存起来，区别就是画线的顺序。
...
{	uint8_t i;
	...
	for(i=0;i<3;i++)
	{
	 ...
	 GUI_Line(Cube_dis[1].x,Cube_dis[1].y,Cube_dis[2].x,Cube_dis[2].y);
	 GUI_Line(Cube_dis[3].x,Cube_dis[3].y,Cube_dis[0].x,Cube_dis[0].y);
	 ...
	 GUI_Line(Cube_dis[1+4].x,Cube_dis[1+4].y,Cube_dis[2+4].x,Cube_dis[2+4].y);
	 GUI_Line(Cube_dis[3+4].x,Cube_dis[3+4].y,Cube_dis[0+4].x,Cube_dis[0+4].y);
	 ...
	 GUI_Line(Triangle_dis[0].x,Triangle_dis[0].y,Triangle_dis[1].x,Triangle_dis[1].y);
	 GUI_Line(Triangle_dis[0].x,Triangle_dis[0].y,Triangle_dis[2].x,Triangle_dis[2].y);
	 GUI_Line(Triangle_dis[1].x,Triangle_dis[1].y,Triangle_dis[2].x,Triangle_dis[2].y);
...
void RateCube(float x,float y,float z,uint16_t XO,uint16_t YO)
{
	float gMAT[4][4];
	uint8_t i;
	_3Dzuobiao temp;
...
//					添加原点设置的形参
///***********************************************************/
void RateCube_all(float x,float y,float z,float mx,float my,float mz,float bx,float by,float bz,int16_t XO,int16_t YO)
{
	float gMAT[4][4];
	uint8_t i;
	_3Dzuobiao temp;
...
//					添加原点设置的形参
///***********************************************************/
void RotatePic32X32(unsigned char *dp,float ax,float ay,float az,uint16_t XO,uint16_t YO)
{
//	int16_t  XO;
//	int16_t  YO;
	uint8_t i,j,k;
...
void Rotatecircle(float ax,float ay,float az,uint16_t r,uint16_t XO,uint16_t YO)
{   
//	int16_t  XO;
//	int16_t  YO;
	uint8_t i;
