...
void RateCube(float x,float y,float z,uint16_t XO,uint16_t YO);//透视投影
void RateCube_all(float x,float y,float z,float mx,float my,float mz,float bx,float by,float bz,int16_t XO,int16_t YO);
void RotatePic32X32(unsigned char *dp,float ax,float ay,float az,uint16_t XO,uint16_t YO);//在三维空间中旋转一个32x32的字符或图形
void RotatePic29X32(unsigned char *dp,float ax,float ay,float az,uint16_t XO,uint16_t YO);
void Rotatecircle(float ax,float ay,float az,uint16_t r,uint16_t XO,uint16_t YO);//在三维空间中旋转一个圆形
#endif
